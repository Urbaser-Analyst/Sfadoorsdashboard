/* =====================================================================
   ...
   - Pick a Customer, then one of that customer's DELIVERED orders.
   ...
   - "Download PDF" snapshots the on-screen template (so the PDF always
     matches what's on screen, including the current logo) via html2canvas
     + jsPDF, paginating automatically if the quotation runs long.
   ===================================================================== */
...
function renderQuotation(DB, f) {
    ...
    <div class="panel">
      ...
      <div class="inline-filterbar">
        <div class="filter-group">
          ...
          <input type="text" id="qCustomerInput" list="qCustomerList" placeholder="Type or pick a customer…" value="${customerOptions.find(c => c.key === quotationState.customerKey)?.name || ""}" ${customerOptions.length ? "" : "disabled"}>
          <datalist id="qCustomerList">${customerOptions.map(c => `<option value="${c.name.replace(/"/g, '&quot;')}">`).join("")}</datalist>
        </div>
        <div class="filter-group">
          <label>Delivered Order</label>
          <select id="qOrderSelect" ${ordersForCustomer.length ? "" : "disabled"}>
            ${ordersForCustomer.length ? ordersForCustomer.map(o => `<option value="${o.orderId}" ${o.orderId === quotationState.orderUid ? "selected" : ""}>${o.orderId} — ${formatDate(o.deliveryDate || o.billDate)} — ${fmtRupee(o.billValue)}</option>`).join("") : `<option>No delivered orders for this customer</option>`}
          </select>
        </div>
      </div>
  ...
  document.getElementById("qCustomerInput").addEventListener("change", (e) => {
    const match = customerOptions.find(c => c.name.toLowerCase() === e.target.value.trim().toLowerCase());
    quotationState.customerKey = match ? match.key : quotationState.customerKey;
    quotationState.orderUid = "";
    renderQuotation(DB, f);
  });
  const orderSelectEl = document.getElementById("qOrderSelect");
  if (orderSelectEl) orderSelectEl.addEventListener("change", (e) => {
    quotationState.orderUid = e.target.value;
...
function renderQuotationBody(DB) {
  ...
  const orderLines = DB.orderLines.filter(l => l.orderUid === bill.orderId);
  ...
  container.innerHTML = `
    ...
    <div class="panel">
      ...
      <div class="table-toolbar" style="margin-bottom:12px">
        ...
        <button type="button" class="btn table-export-btn" id="qDownloadPdfBtn">Download PDF</button>
      </div>
        ...
        <div class="quotation-sheet" id="quotationSheet">
          ...
          <table class="q-items-table">
            ...
            <tbody>
              ...
              </tr>`).join("") || `<tr><td colspan="5" class="empty-note">No line items found for this order.</td></tr>`}
            </tbody>
          </table>
  ...
  document.getElementById("qDownloadPdfBtn").addEventListener("click", () => {
