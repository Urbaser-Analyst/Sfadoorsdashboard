...
const CHART_PALETTE = ["#0F766E", "#1976A8", "#14B8A6", "#0E7490", "#2DD4BF", "#38BDF8", "#115E59", "#67E8F9"];
const chartInstances = {};

Chart.defaults.font.family = "'Manrope', system-ui, sans-serif";
Chart.defaults.color = "#486581";
Chart.defaults.borderColor = "#CFEDEA";
...
// "Rs." prefix (no special ₹ glyph) for customer-facing plain-text messages —
// SMS/WhatsApp-safe, and shows 2 decimals only when the amount actually has paise.
function fmtRupeeText(n) {
  if (n === null || n === undefined || isNaN(n)) return "Rs.0";
  const rounded = Math.round(Number(n) * 100) / 100;
  const hasPaise = Math.abs(rounded % 1) > 0.001;
  return "Rs." + rounded.toLocaleString("en-IN", { minimumFractionDigits: hasPaise ? 2 : 0, maximumFractionDigits: 2 });
}
...
function makeLineChart(canvasId, labels, datasets, opts = {}) {
  ...
  chartInstances[canvasId] = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: { beginAtZero: true, ticks: { callback: v => fmtINR(v) }, grid: { color: "#DDF3F0" } },
