...
function renderSummary(DB, f) {
  ...
  const salesValue = deliveredBills.reduce((s, b) => s + b.billValue, 0);
  ...
  const monthly = {};
  bills.forEach(b => { const k = monthKey(b.deliveryDate || b.billDate); (monthly[k] = monthly[k] || { sale: 0, pay: 0, ret: 0 }).sale += b.billValue; });
  ...
  document.getElementById("viewRoot").innerHTML = `
    <div class="kpi-grid">
      ...
      ${kpiCard("In Progress Value", fmtRupee(inProgressValue), inProgressBills.length + " not yet delivered · all delivery dates", "amber")}
    ...
    <p class="small-note" style="margin-top:-10px;margin-bottom:16px">"Sales Value" only counts orders with Delivery Status = Delivered. Approved / Verified / Production Completed orders are tracked separately as "In Progress" and excluded from sales figures.</p>
...
function renderCreditPaid(DB, f) {
  ...
  document.getElementById("viewRoot").innerHTML = `
    ...
    <div class="panel">
      <div class="panel-header"><div class="panel-title">Product-wise Average Price — Credit vs Paid (by month)</div><div class="panel-note">Selling rate often differs between credit and cash orders for the same product</div></div>
