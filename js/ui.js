...
function renderSummary(DB, f) {
  ...
  const pipelineKeys = ["approvalStatus", "verificationStatus", "productionStatus", "deliveryStatus"];
  const pipelineData = pipelineKeys.map(k => {
    const counts = groupSum(bills, b => b[k] || "Unknown", () => 1);
    return { stage: k.replace("Status", ""), counts };
  });

  const custRows = summarizeBillsByCustomer(bills).sort((a, b) => b.pendingTotal - a.pendingTotal).slice(0, 10);
  const productAgg = {};
  ...
  document.getElementById("viewRoot").innerHTML = `
    <div class="kpi-grid">
      ${kpiCard("Total Order Value", fmtRupee(totalBilled), bills.length + " bills")}
      ${kpiCard("Total Collected", fmtRupee(totalCollected), payments.length + " payments", "sage")}
      ${kpiCard("Net Outstanding", fmtRupee(totalOutstanding), activeCustomers + " customers with bills", "clay")}
    ...
    <div class="grid-2">
      ...
      <div class="panel">
        <div class="panel-header"><div class="panel-title">Order Pipeline</div><div class="panel-note">Bills by workflow stage</div></div>
  ...
  const stageLabels = pipelineData.map(p => p.stage);
  const allStatuses = Array.from(new Set(pipelineData.flatMap(p => Object.keys(p.counts))));
  makeBarChart("chartPipeline", stageLabels, allStatuses.map((status, i) => ({
    label: status, data: pipelineData.map(p => p.counts[status] || 0), stack: "s"
  })), { horizontal: true, options: { scales: { x: { stacked: true }, y: { stacked: true } } } });
...
function renderOutstanding(DB, f) {
  const bills = filterBills(DB.bills, f);
  const custRows = summarizeBillsByCustomer(bills).sort((a, b) => b.pendingTotal - a.pendingTotal);
  ...
  renderDataTable("custTable", {
    ...
    columns: [
      ...
      { key: "pendingTotal", label: "Outstanding", numeric: true, render: r => `<strong>${fmtRupee(r.pendingTotal)}</strong>` },
...
function renderProducts(DB, f) {
  ...
  const inScope = lines.filter(l => filteredOrderIds.has(l.orderUid));
  ...
  document.getElementById("viewRoot").innerHTML = `
    <div class="grid-2">
      <div class="panel">
        <div class="panel-header"><div class="panel-title">Top Products by Quantity Sold</div><div class="panel-note">Irrespective of dimensions / colors / designs</div></div>
...
function renderRaw(DB, f) {
  ...
  renderDataTable("rawOrders", { columns: [
    ...
    { key: "billDate", label: "Date", render: r => formatDate(r.billDate) }, { key: "billValue", label: "Value", numeric: true, render: r => fmtRupee(r.billValue) },
    { key: "deliveryMode", label: "Delivery Mode" }, { key: "deliveryStatus", label: "Delivery Status" }
