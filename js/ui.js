...
const uomCalcState = { sae: new Set(), customer: new Set() };
...
function renderUomRateCalc(DB, deliveredLines) {
  ...
  lines.forEach(l => {
    ...
    // different rate) collapses into one indistinguishable row.
    const key = l.billName || l.productName;
    const p = (map[key] = map[key] || {
      productName: key, uom: l.uom || "Unit",
      creditQty: 0, creditComputedQty: 0, creditAmt: 0,
      paidQty: 0, paidComputedQty: 0, paidAmt: 0
    });
    const computedQty = uomComputedQty(l);
    const prefix = l.saleType === "Credit Sale" ? "credit" : "paid";
    p[`${prefix}Qty`] += l.finalQty;
    p[`${prefix}ComputedQty`] += computedQty;
    p[`${prefix}Amt`] += l.amount;
  });
  const rows = Object.values(map).map(p => ({
    ...p,
    creditAvgRate: p.creditComputedQty > 0 ? p.creditAmt / p.creditComputedQty : null,
    paidAvgRate: p.paidComputedQty > 0 ? p.paidAmt / p.paidComputedQty : null,
    totalComputedQty: p.creditComputedQty + p.paidComputedQty,
    totalAmt: p.creditAmt + p.paidAmt
  })).sort((a, b) => b.totalAmt - a.totalAmt);

  const totals = rows.reduce((acc, p) => {
    acc.creditQty += p.creditQty; acc.creditComputedQty += p.creditComputedQty; acc.creditAmt += p.creditAmt;
    acc.paidQty += p.paidQty; acc.paidComputedQty += p.paidComputedQty; acc.paidAmt += p.paidAmt;
    return acc;
  }, { creditQty: 0, creditComputedQty: 0, creditAmt: 0, paidQty: 0, paidComputedQty: 0, paidAmt: 0 });
  ...
  const topByComputedQty = rows.slice().sort((a, b) => b.totalComputedQty - a.totalComputedQty).slice(0, 8);

  const container = document.getElementById("uomRateCalcSection");
  if (!container) return;
  container.innerHTML = `
    <div class="panel-header">
      <div class="panel-title">UOM Rate Calculations — Credit vs Cash (${rows.length} products)</div>
      <div class="panel-note">Delivered orders only. Sold quantity is computed per product's UOM: SQ.ft = H×W×Final Qty, SQft = (H×W÷144)×Final Qty, otherwise Final Qty as-is. Avg Rate = Amount ÷ computed quantity.</div>
    </div>
    ...
    <div class="kpi-grid" style="margin-bottom:16px">
      ${kpiCard("Credit Computed Qty", totals.creditComputedQty.toLocaleString("en-IN", { maximumFractionDigits: 1 }), fmtRupee(totals.creditAmt), "clay")}
      ${kpiCard("Cash Computed Qty", totals.paidComputedQty.toLocaleString("en-IN", { maximumFractionDigits: 1 }), fmtRupee(totals.paidAmt), "sage")}
      ${kpiCard("Credit Avg Rate", totals.creditComputedQty > 0 ? fmtRupee(totals.creditAmt / totals.creditComputedQty) : "—", "per computed unit")}
      ${kpiCard("Cash Avg Rate", totals.paidComputedQty > 0 ? fmtRupee(totals.paidAmt / totals.paidComputedQty) : "—", "per computed unit")}
    </div>
    <div class="table-scroll">
      <table class="data-table">
        <thead>
          <tr>
            <th rowspan="2">Product Name</th><th rowspan="2">UOM</th>
            <th colspan="3" style="text-align:center;background:#fdecea">Credit</th>
            <th colspan="3" style="text-align:center;background:#eaf6ee">Cash</th>
          </tr>
          <tr>
            <th class="num">Qty</th><th class="num">Computed Qty</th><th class="num">Amount</th>
            <th class="num">Qty</th><th class="num">Computed Qty</th><th class="num">Amount</th>
          </tr>
        </thead>
        <tbody>
          ${rows.map(p => `<tr>
            <td>${p.productName}</td><td>${p.uom}</td>
            <td class="num">${p.creditQty.toLocaleString("en-IN")}</td>
            <td class="num">${p.creditComputedQty.toLocaleString("en-IN", { maximumFractionDigits: 1 })}</td>
            <td class="num">${fmtRupee(p.creditAmt)}</td>
            <td class="num">${p.paidQty.toLocaleString("en-IN")}</td>
            <td class="num">${p.paidComputedQty.toLocaleString("en-IN", { maximumFractionDigits: 1 })}</td>
            <td class="num">${fmtRupee(p.paidAmt)}</td>
          </tr>`).join("") || `<tr><td colspan="8" class="empty-note">No delivered lines match the current filters.</td></tr>`}
        </tbody>
        <tfoot>
          <tr style="font-weight:700;background:var(--surface-2)">
            <td colspan="2">Total</td>
            <td class="num">${totals.creditQty.toLocaleString("en-IN")}</td>
            <td class="num">${totals.creditComputedQty.toLocaleString("en-IN", { maximumFractionDigits: 1 })}</td>
            <td class="num">${fmtRupee(totals.creditAmt)}</td>
            <td class="num">${totals.paidQty.toLocaleString("en-IN")}</td>
            <td class="num">${totals.paidComputedQty.toLocaleString("en-IN", { maximumFractionDigits: 1 })}</td>
            <td class="num">${fmtRupee(totals.paidAmt)}</td>
          </tr>
        </tfoot>
      </table>
    </div>
  ...
  makeBarChart("chartUomTopProducts", topByComputedQty.map(p => p.productName.slice(0, 20)),
    [{ label: "Computed Qty", data: topByComputedQty.map(p => p.totalComputedQty), backgroundColor: CHART_PALETTE[2] }],
    { horizontal: true });
