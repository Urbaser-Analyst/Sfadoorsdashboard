...
let ledgerStatusFilter = "all"; // local to Bill-wise Ledger; never affects global filters or other reports
...
function renderSummary(DB, f) {
  ...
  document.getElementById("viewRoot").innerHTML = `
    <div class="kpi-grid">
      ...
      ${kpiCard("Outstanding (Incl. Surplus)", fmtRupee(totalOutstandingIncludingSurplus), "Excl. Surplus − Excess Received")}
      ${kpiCard("Excess Received Amount", fmtRupee(totalSurplus), "customer surplus / advances", "amber")}
  ...
  document.getElementById("summaryExportBtn").addEventListener("click", () => downloadExcelFile("executive_summary", [
    ...
    { metric: "Outstanding (Incl. Surplus)", value: totalOutstandingIncludingSurplus, detail: "Outstanding Excl. Surplus − Excess Received Amount" },
    { metric: "Excess Received Amount", value: totalSurplus, detail: "customer surplus / advances" },
...
function renderOutstanding(DB, f) {
  ...
  const customerRows = summarizeBillsByCustomer(filteredBills, filterPayments(DB.payments, f), filterReturns(DB.stockReturns, f));
  const customerOutstandingExcl = customerRows.reduce((s, c) => s + c.outstandingExcludingSurplus, 0);
  const customerExcessReceived = customerRows.reduce((s, c) => s + c.surplus, 0);
  const customerOutstandingIncl = customerRows.reduce((s, c) => s + c.outstandingIncludingSurplus, 0);

  document.getElementById("viewRoot").innerHTML = `
    <div class="kpi-grid">
      ${kpiCard("Order Value", fmtRupee(totalBillValue), `${rows.length} orders`)}
      ${kpiCard("Adjustments", fmtRupee(totalAdjustments), "allocated payments + returns", "sage")}
      ${kpiCard("Order Balance", fmtRupee(totalBalance), `${outstandingCount} outstanding orders`, "clay")}
      ${kpiCard("Outstanding (Excl. Surplus)", fmtRupee(customerOutstandingExcl), "positive customer net balances", "clay")}
      ${kpiCard("Excess Received Amount", fmtRupee(customerExcessReceived), "customer surplus / advances", "amber")}
      ${kpiCard("Outstanding (Incl. Surplus)", fmtRupee(customerOutstandingIncl), "Excl. Surplus − Excess Received", "sage")}
    ...
    <div class="panel">
      <div class="panel-header">
        ...
        <div class="panel-note">Payments and stock returns are allocated to each customer’s oldest orders first (FIFO). Balance is order-level; Outstanding Excl./Incl. Surplus and Excess Received are customer-level reconciliations.</div>
      </div>
...
function renderLedger(DB, f) {
  ...
  const tableBills = bills.filter(b => ledgerStatusFilter === "all" || (ledgerStatusFilter === "delivered" ? b.isDelivered : !b.isDelivered));

  // The Delivered/In Progress slicer is table-only. KPI cards remain tied to
  // the global filters so the status toggle cannot silently change totals.
  ...
  const ledgerCustomers = summarizeBillsByCustomer(bills, filterPayments(DB.payments, f), filterReturns(DB.stockReturns, f));
  const ledgerOutstandingExcl = ledgerCustomers.reduce((s, c) => s + c.outstandingExcludingSurplus, 0);
  const ledgerExcessReceived = ledgerCustomers.reduce((s, c) => s + c.surplus, 0);
  const ledgerOutstandingIncl = ledgerCustomers.reduce((s, c) => s + c.outstandingIncludingSurplus, 0);
  ...
  document.getElementById("viewRoot").innerHTML = `
    <div class="kpi-grid">
      ...
      ${kpiCard("Order Balance", fmtRupee(outstanding), pendingOrders + " orders pending", "clay")}
      ${kpiCard("Outstanding (Excl. Surplus)", fmtRupee(ledgerOutstandingExcl), "positive customer net balances", "clay")}
      ${kpiCard("Excess Received Amount", fmtRupee(ledgerExcessReceived), "customer surplus / advances", "amber")}
      ${kpiCard("Outstanding (Incl. Surplus)", fmtRupee(ledgerOutstandingIncl), "Excl. Surplus − Excess Received", "sage")}
      ${kpiCard("No. of Orders", bills.length, "global-filtered orders")}
      ${kpiCard("Pending Orders", pendingOrders, "global-filtered orders")}
    </div>
    <div class="panel">
      <div class="panel-header">
        <div class="panel-title">Bill-wise Ledger (${tableBills.length})</div>
        <div class="panel-note">Every order = one bill. Pending balance = bill value − allocated payments − allocated returns (oldest-bill-first). Showing ${tableBills.length} of ${bills.length} global-filtered orders.</div>
      </div>
      <div class="local-filter-row">
        <label class="local-filter-label" for="ledgerStatusFilter">Order Status</label>
        <select id="ledgerStatusFilter" class="local-filter-select">
          <option value="all" ${ledgerStatusFilter === "all" ? "selected" : ""}>All Orders</option>
          <option value="delivered" ${ledgerStatusFilter === "delivered" ? "selected" : ""}>Delivered Only</option>
          <option value="inprogress" ${ledgerStatusFilter === "inprogress" ? "selected" : ""}>In Progress Only</option>
        </select>
        <span class="small-note">This slicer applies only to Bill-wise Ledger.</span>
      </div>
      <div id="ledgerTable"></div>
    </div>
  `;

  document.getElementById("ledgerStatusFilter").addEventListener("change", e => {
    ledgerStatusFilter = e.target.value;
    renderLedger(DB, f);
  });
