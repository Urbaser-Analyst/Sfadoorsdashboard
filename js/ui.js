...
function renderOutstanding(DB, f) {
  ...
  const masterKeys = masterList.length ? masterList : customerCalc.map(c => ({ customerName: c.customerName, customerKey: c.customerKey }));
...
function openCustomerLedger(DB, customerKey, highlightOrderId) {
  ...
  const statementEvents = [
    ...bills.map(b => ({ type: b.isDelivered ? "Delivered" : "Not Delivered", ref: "Order", date: b.billDate, id: b.orderId, quotationNo: b.quotationNo || "—", value: b.billValue, orderValue: b.billValue, receiptValue: 0, customerName: b.customerName, mobile: b.mobile || "", sae: b.sae })),
    ...payments.map(p => ({ type: "Payment", ref: "Payment", date: p.date, id: p.id, quotationNo: "—", value: p.amount, orderValue: 0, receiptValue: p.amount, customerName: p.customerName, mobile: p.mobile || "", sae: p.sae })),
    ...returns.map(r => ({ type: "Return", ref: "Return", date: r.date, id: r.id, quotationNo: "—", value: r.amount, orderValue: 0, receiptValue: r.amount, customerName: r.customerName, mobile: r.mobile || "", sae: r.sae }))
  ].sort((a, b) => (a.date || 0) - (b.date || 0));
  let statementOrders = 0, statementReceipts = 0;
  const statementRows = statementEvents.map(e => {
    statementOrders += e.orderValue; statementReceipts += e.receiptValue;
    const balance = +(statementOrders - statementReceipts).toFixed(2);
    const mobile = e.mobile || bills.find(b => b.mobile)?.mobile || "—";
    const action = e.ref === "Payment" ? `We have received a payment of ${fmtRupee(e.value)} on ${formatDate(e.date)}.` : e.ref === "Return" ? `We have received stock return of ${fmtRupee(e.value)} on ${formatDate(e.date)}.` : `Your bill ${e.id} dated ${formatDate(e.date)} has been generated for ${fmtRupee(e.value)}.`;
    return { ...e, mobile, cumulativeOrderValue: +statementOrders.toFixed(2), cumulativeReceipts: +statementReceipts.toFixed(2), balance, message: `${action} Updated balance: ${fmtRupee(balance)}.` };
  });
  ...
  openModal(`
    ...
    <div class="panel-title" style="margin-bottom:8px">Each Entry Statement</div>
    <div id="modalStatementTable" style="margin-bottom:20px"></div>
  ...
  renderDataTable("modalStatementTable", {
    searchPlaceholder: "Search statement…", pageSize: 10, exportName: "customer_entry_statement",
    columns: [
      { key: "quotationNo", label: "Quotation No" }, { key: "customerName", label: "Customer Name" }, { key: "type", label: "Type", render: r => statusTag(r.type === "Delivered" ? "Settled" : r.type === "Payment" ? "Paid" : r.type) },
      { key: "date", label: "Date", render: r => formatDate(r.date) }, { key: "value", label: "Value", numeric: true, render: r => fmtRupee(r.value) }, { key: "ref", label: "Reference" }, { key: "mobile", label: "Mobile" },
      { key: "cumulativeOrderValue", label: "Order Value", numeric: true, render: r => fmtRupee(r.cumulativeOrderValue) }, { key: "cumulativeReceipts", label: "Payments + Return", numeric: true, render: r => fmtRupee(r.cumulativeReceipts) }, { key: "balance", label: "Outstanding Balance", numeric: true, render: r => fmtRupee(r.balance) }, { key: "message", label: "Message" }, { key: "sae", label: "SAE" }
    ], rows: statementRows
  });
