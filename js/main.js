...
function populateFilterOptions(DB) {
  ...
  const modeSelect = document.getElementById("f_deliveryMode");
  modeSelect.innerHTML = `<option value="">All</option>` + DB.deliveryModeList.map(m => `<option value="${m}">${m}</option>`).join("");
  ...
  datalist.innerHTML = DB.customerNameList.map(c => `<option value="${c}">`).join("");
}
...
function wireControls() {
  ...
  const filterIds = ["f_dateFrom", "f_dateTo", "f_customerSearch", "f_sae", "f_deliveryMode", "f_minValue", "f_paymentVia"];
  ...
  document.getElementById("clearFiltersBtn").addEventListener("click", () => {
    ...
    document.getElementById("f_deliveryMode").value = "";
