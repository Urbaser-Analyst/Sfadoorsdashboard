...
function buildModel(raw) {
  ...
  const orders = raw.order.rows
    .filter(r => cleanStr(r[oCols.id]) || cleanStr(r[oCols.shortId]))
    .map(r => {
      const orderDate = parseFlexibleDate(r[oCols.timestamp]);
      // Sales/date filtering uses the Order sheet's Delivery Date.
      // DD Timestamp is not a delivery date and is intentionally not used here.
      const deliveryDate = parseFlexibleDate(r[oCols.deliveryDate]);
      const billDate = deliveryDate || orderDate; // ageing fallback only
      return {
        orderId: cleanStr(r[oCols.id]) || cleanStr(r[oCols.shortId]),
        // Normalized join key (trimmed, whitespace-collapsed, case-folded) —
        // used for ALL matching against Order List's "Order UID". Manually
        // maintained spreadsheets routinely have a stray double-space or a
        // casing difference between the two sheets; matching on the raw
        // string (as this used to) silently drops those orders' product
        // lines everywhere they're joined — Quotation, Product Analysis,
        // UOM Rate Calc, etc. orderId itself is kept untouched for display.
        orderKey: normName(r[oCols.id]) || normName(r[oCols.shortId]),
        // Short internal bill/order reference (e.g. "ca4bb5c4"), distinct from
        // the long descriptive Order ID. Falls back to the Order ID itself if
        // the sheet has no separate short "ID" column.
        billId: cleanStr(r[oCols.shortId]) || cleanStr(r[oCols.id]),
        // Normalized form of billId. Some sheets populate Order List's
        // "Order UID" from this "ID" column instead of "Order ID" — see the
        // dual-key index built below, which matches against either one.
        billKey: normName(r[oCols.shortId]) || normName(r[oCols.id]),
        ...
        orderDate,
        deliveryDate,
        billDate, // the date the bill "ages" from
        ...
        transportCharges: toNumber(r[oCols.transport]),
        packingCharges: toNumber(r[oCols.packing]),
        loadingCharges: toNumber(r[oCols.loading]),
        otherCharges: toNumber(r[oCols.other])
      };
    });
  ...
  const olCols = {
    ...
    sticker: pick(olMap, "Sticker"),
    fullText: pick(olMap, "Full Text"),
    billName: pick(olMap, "Bill Name"),
    qty: pick(olMap, "Qty"),
  ...
  // Order List's "Order UID" is sometimes populated from the Orders sheet's
  // "Order ID" column and sometimes from its short "ID" column, depending on
  // how the sheet/form was set up — so index BOTH keys to the same order.
  // orderKey ("Order ID") is inserted first and wins on a clash; billKey
  // ("ID") only fills in gaps, since a genuine collision between two
  // different orders' short hash IDs and long Order IDs is effectively
  // impossible.
  const orderByIdIndex = {};
  orders.forEach(o => { orderByIdIndex[o.orderKey] = o; });
  orders.forEach(o => { if (o.billKey && !(o.billKey in orderByIdIndex)) orderByIdIndex[o.billKey] = o; });

  const orderLines = raw.orderList.rows
    .filter(r => cleanStr(r[olCols.orderUid]))
    .map(r => {
      const orderUid = cleanStr(r[olCols.orderUid]);
      const orderKey = normName(r[olCols.orderUid]);
      const parent = orderByIdIndex[orderKey];
      // Use Final Qty as the source of truth. Fall back to Qty only when
      // Final Qty is blank; a legitimate numeric zero must remain zero.
      const finalQtyCell = cleanStr(r[olCols.finalQty]);
      const finalQty = finalQtyCell === "" ? toNumber(r[olCols.qty]) : toNumber(finalQtyCell);
      const uom = cleanStr(r[olCols.uom]);
      const height = toNumber(r[olCols.height]);
      const width = toNumber(r[olCols.width]);
      // UOM-based sold-quantity formulas (delivered-order reporting only):
      //   "SQ.ft"  — H and W are already in feet (e.g. an 8x4 plywood board): H × W × Final Qty
      //   "SQft"   — H and W are in inches (e.g. an 81x32 door): (H × W ÷ 144) × Final Qty
      //   anything else ("Unit", "Pcs", blank, ...) — no area to compute; Final Qty is the unit count
      let uomQty = null;
      if (height > 0 && width > 0) {
        if (uom.toUpperCase() === "SQ.FT") uomQty = height * width * finalQty;
        else if (uom.toUpperCase() === "SQFT") uomQty = ((height * width) / 144) * finalQty;
      }
      return {
        id: cleanStr(r[olCols.id]),
        orderUid,
        // Canonicalize to the PARENT BILL's own orderKey (from its "Order
        // ID" column) whenever a parent was found via either key — not this
        // line's own normalized Order UID. Every downstream comparison
        // (Quotation, Product Analysis, Credit vs Paid, ...) matches lines
        // to bills via `l.orderKey === bill.orderKey`; if a line joined only
        // through the billKey fallback (Order UID = the sheet's short "ID",
        // not "Order ID"), its own orderKey would otherwise stay a
        // different string from the bill's, and every one of those
        // comparisons would silently fail again despite the join above
        // having actually found the right parent.
        orderKey: parent ? parent.orderKey : orderKey,
        ...
        sticker: cleanStr(r[olCols.sticker]),
        fullText: cleanStr(r[olCols.fullText]),
        billName: cleanStr(r[olCols.billName]) || cleanStr(r[olCols.product]) || "Unspecified",
  ...
  const pCols = {
    ...
    sae: pick(pMap, "SAE"),
    cid: pick(pMap, "CID"),
    mobile: pick(pMap, "Mobile No", "Mobile Number", "Customer Mobile Number"),
  ...
  const payments = raw.payments.rows
    ...
    .map(r => ({
      ...
      // Use the Payments sheet's explicit Date column for the slicer, charts,
      // display, and payment allocation. Timestamp is entry/modify time,
      // not the business date of the payment.
      date: parseFlexibleDate(r[pCols.date]),
      sae: cleanStr(r[pCols.sae]) || "Unassigned",
      cid: cleanStr(r[pCols.cid]),
      mobile: cleanStr(r[pCols.mobile]),
  ...
  const srCols = {
    ...
    sae: pick(srMap, "SAE"),
    cid: pick(srMap, "CID"),
    mobile: pick(srMap, "Mobile No", "Mobile Number", "Customer Mobile Number"),
  ...
  const stockReturns = raw.stockReturn.rows
    ...
    .map(r => ({
      ...
      // Stock Return date filters and reporting use the Stock Return sheet's Date column.
      date: parseFlexibleDate(r[srCols.date]),
      sae: cleanStr(r[srCols.sae]) || "Unassigned",
      cid: cleanStr(r[srCols.cid]),
      mobile: cleanStr(r[srCols.mobile]),
  ...
  allCustomerKeys.forEach(key => {
    ...
    custBills.forEach(b => {
      ...
      const adjustments = +(paidAmt + returnedAmt).toFixed(2);
      // Credits are processed chronologically. The settlement date is the date
      // of the allocation that finally clears this order; for outstanding orders
      // it remains blank and ageing is measured through today.
      const lastAllocDate = b.allocations.length ? b.allocations.reduce((m, a) => (a.date > m ? a.date : m), b.allocations[0].date) : null;
      const settlementDate = pending <= 0.5 ? (lastAllocDate || b.billDate) : null;
      const ageingDays = settlementDate ? Math.max(0, daysBetween(b.billDate, settlementDate)) : Math.max(0, daysAgo(b.billDate));
      const ageDays = pending > 0.5 ? daysAgo(b.billDate) : null;
      let status = "Outstanding";
      if (pending <= 0.5) status = "Settled";
      else if (adjustments > 0.5) status = "Partially Settled";
      ...
      bills.push({
        ...
        transportCharges: +b.transportCharges.toFixed(2),
        packingCharges: +b.packingCharges.toFixed(2),
        loadingCharges: +b.loadingCharges.toFixed(2),
        otherCharges: +b.otherCharges.toFixed(2),
        ...
        settlementDate,
        status,
        billType: b.saleType === "Paid Sale" ? "Paid" : "Credit",
        saleType,
  ...
  // Same dual-key matching as orderByIdIndex above: try orderKey first (from
  // "Order ID"), then billKey (from "ID") as a fallback for orders where
  // Order List's "Order UID" was populated from the short ID instead.
  const billByOrderId = {};
  bills.forEach(b => { billByOrderId[b.orderKey] = b; });
  bills.forEach(b => { if (b.billKey && !(b.billKey in billByOrderId)) billByOrderId[b.billKey] = b; });
  orderLines.forEach(l => {
    const parentBill = billByOrderId[l.orderKey];
  ...
  const ensureCustomer = (key, name) => (customerSummaryMap[key] = customerSummaryMap[key] || {
  ...
  bills.forEach(b => {
    ...
    const c = ensureCustomer(b.customerKey, b.customerName);
    c.totalBilled += b.billValue; c.billCount += 1; c.saeSet.add(b.sae);
  ...
  payments.forEach(p => {
    const c = ensureCustomer(p.customerKey, p.customerName);
    c.totalPaid += p.amount;
    if (!c.lastPaymentDate || p.date > c.lastPaymentDate) c.lastPaymentDate = p.date;
    if (p.sae) c.saeSet.add(p.sae);
  });
  stockReturns.forEach(r => {
    const c = ensureCustomer(r.customerKey, r.customerName);
  ...
  const customerSummaries = Object.values(customerSummaryMap).map(c => {
    ...
    return {
      ...
      outstandingIncludingSurplus: netOutstanding, surplus: +surplus.toFixed(2), netOutstanding,
  ...
  orderLines.forEach(l => {
    ...
    if (l.isDelivered) {
      p.qtySold += l.finalQty;
      p.amountSold += l.amount;
      p.orderCount += 1;
    } else {
      p.qtyInProgress += l.finalQty;
  ...
  return {
    ...
    dimensionList, subTypeList, designList, majorColorList, minorColorList, productNameList,
    customerMasterList,
